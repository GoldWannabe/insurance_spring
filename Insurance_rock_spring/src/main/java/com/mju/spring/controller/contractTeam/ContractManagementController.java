...
import com.mju.spring.exception.AlreadyApplyRenewException;
...
public class ContractManagementController {
	...
	public String selectContract(HttpServletRequest request, Model model) {
		...
		if (!accidentList.isEmpty() && insuranceDetailsDto != null) {
			...
			throw new AlreadyApplyRenewException();
		}
	...
	public String applyRenew(HttpServletRequest request, Model model) {
		// 계약 관리 할 때, 갱신 부분을 할 경우 rank 업데이트 하지 말고 현재 랭크 id 맨 앞에 *붙여서 새로 저장 //이미 갱신완료된
		// 계약의 경우 제외할것.
...
//	E2. DB 접근에 실패한 경우
	...
	private ModelAndView alreadyApplyRenewException(Exception e) {
		...
		ModelAndView modelAndView = new ModelAndView();
		modelAndView.setViewName("menu//showResult");
		modelAndView.addObject("JudgeResult", "해당 계약은 이미 갱신 신청 접수가 되어 갱신 혹은  해지할수없습니다. 다른 계약을 선택하시거나 고객센터(010-1234-5678)로 문의주기실 바랍니다.");
		return modelAndView;
	}
